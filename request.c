...
{
	t_stats->total_req++; // Increment total request count when thread starts handling a request, againts logic, but that's what the course expects
    ...
    if (strcasecmp(method, "GET") == 0) {
		...
		memset(log_buf, 0, MAXLINE);

        gettimeofday(&tm_stats.log_enter, NULL); // 2026 code

        writer_lock(log);

        if (log->debug_sleep_time > 0) sleep(log->debug_sleep_time); // 2026 new feature

        gettimeofday(&tm_stats.log_exit, NULL); // 2026 code

		int buf_len = append_stats(log_buf, t_stats, tm_stats);

		add_to_log(log, log_buf, buf_len);
